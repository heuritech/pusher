...
def send_info(addr_gateway, name_job, name_metric, source=None, block=None,
              family=None, value=None, add=None):
    """
    ...
    @add : Get the metric on @addr_gateway
        and add '@add' (@value not used, overight by the value on @addr_gateway)
    """
    if "HOST" in os.environ:
        name_job += "_" + os.environ["HOST"]
    if addr_gateway is None:
        addr_gateway = os.environ.get("PROMETHEUS_HOST")
    ...
    try:
        ...
        logging.error("Metric[{0}]: {1} with labels {2}".format(name_metric,
                                                                str(value),
                                                                str(dict_labels)))
